...
import { ComponentLibraryPanel } from "@/components/design/ComponentLibraryPanel";
...
export function CodeViewer() {
  ...
  const [sidebarTab, setSidebarTab] = useState<"files" | "components">("files");
  ...
  return (
    <div className="flex h-full bg-[#111118] overflow-hidden">
      <div className="w-[240px] shrink-0 h-full flex flex-col bg-[#1a1a24] border-r border-[#1e1e2e]">
        {/* Sidebar tab strip */}
        <div className="flex shrink-0 border-b border-[#1e1e2e]">
          {(["files", "components"] as const).map((tab) => (
            <button
              key={tab}
              onClick={() => setSidebarTab(tab)}
              className={cn(
                "flex-1 py-1.5 text-[10px] uppercase tracking-[0.14em] font-medium transition-colors",
                sidebarTab === tab
                  ? "text-primary border-b border-primary bg-primary/5"
                  : "text-[#5a5a75] hover:text-[#8080a0]",
              )}
            >
              {tab === "files" ? "Files" : "Components"}
            </button>
          ))}
        </div>

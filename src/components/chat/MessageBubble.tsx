...
function parseAssistantMessage(raw: string): {
  ...
  // Strip AI design brief preamble (✦ Design Brief...✦ Build) — internal planning, not for chat UI
  if (text.includes("✦ Design Brief")) {
    text = text.replace(/✦\s*Design Brief[\s\S]*/g, "").trim();
  }
  // Also strip orphaned ✦ section headers from partial renders
  text = text.replace(/✦\s*(?:Architecture|Sections Plan|Build)[^\n]*\n[\s\S]*/g, "").trim();

  // Strip injected market/site research context blocks from user message display
  text = text
    .replace(/\[MARKET RESEARCH:[^\]]*\]\n[\s\S]*?(?=\n\nUser request:|\s*$)/g, "")
    .replace(/\[SITE DNA:[^\]]*\]\n[\s\S]*?(?=\n\nUser request:|\s*$)/g, "")
    .replace(/\[MARKET CONTEXT:[^\]]+\]/g, "")
    .replace(/\[MARKET NOTE:[^\]]+\]/g, "")
    .replace(/\[CURRENT CODEBASE[^\]]*\][\s\S]*?\[END CODEBASE\]/g, "")
    .trim();
...
function cleanUserDisplay(content: string): string {
  return content
    .replace(/\[MARKET CONTEXT:[^\]]+\]/g, "")
    .replace(/\[MARKET NOTE:[^\]]+\]/g, "")
    .replace(/\[SITE DNA:[^\]]*\][^\n]*\n[\s\S]*?(?=\n\nUser request:)/g, "")
    ...
    .replace(/\n\nUser request:\s*/g, "")
    .replace(/\[CURRENT CODEBASE[\s\S]*?\[END CODEBASE\]/g, "")
    .replace(/<<BRAND_MASTER_CONTEXT>>[\s\S]*?<<END_BRAND_MASTER_CONTEXT>>/g, "")
    .replace(/\[BRAND_MASTER_CONTEXT\][\s\S]*?\[END_BRAND_MASTER_CONTEXT\]/g, "")
    .replace(/\[CODEBASE CONTEXT[\s\S]*?\[END CODEBASE CONTEXT\]/g, "")
    .trim();
}
...
export function MessageBubble({ message }: { message: ChatMessage }) {
  ...
  return (
    ...
    >
      ...
      >
        {isUser ? (
          <p className="whitespace-pre-wrap">{cleanUserDisplay(message.content)}</p>
        ) : (
